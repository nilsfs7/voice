...
export default async function PollPage({ params }: Ctx) {
  ...
  const commentAuthors = await fetchFsmeetUsers(
    comments.map((c) => c.author_username),
  );

  let ageGate: { ok: true } | { ok: false; reason: string; missing: string[] } =
    { ok: true };
  if (user) {
  ...
  const userCanScore = Boolean(
    user && canScore(user.type) && ageGate.ok,
  );
    ...
    <div className="space-y-8">
      <div className="space-y-4">
        <div className="flex items-center gap-3 text-sm text-text-muted">
          ...
          >
            ...
            <span className="fsmeet-profile-name font-medium text-text">
              {creator ? displayName(creator) : poll.creator_username}
            </span>
          </FsmeetProfileTrigger>
          <span>·</span>
          <span>
            ...
            {poll.choice_mode === "single"
              ? messages.poll.single
              : messages.poll.multiple}
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-3 text-sm text-text-muted">
          <span>
            {formatCount(messages.home.totalVotes, { count: totalVotes })}
          </span>
          <span>Score {Number(poll.score ?? 0)}</span>
          <span>
            {new Date(poll.start_at).toLocaleString("en-GB")} →{" "}
            {new Date(poll.end_at).toLocaleString("en-GB")}
          </span>
        </div>
        <PollAudienceRules
        ...
        {poll.status === "published" ? (
          <SharePollButton url={shareUrl} />
        ) : null}
        {isCreator ? (
          <PollActions
            publicId={poll.alias || poll.public_id}
            status={poll.status}
            canEdit={poll.status === "draft" && canCreatePoll(user!.type)}
          />
        ) : null}
      ...
      {poll.status === "published" ? (
        ...
        <div className="card p-5 text-sm text-text-muted">
          This draft is only visible to you. Publish it when ready.
        </div>
      )}
