...
| TECH-15 | Site-wide SEO metadata: **`metadataBase`** from site URL; root metadata includes description, **Open Graph**, and **Twitter** card | must | accepted | `metadataBase` from `getSiteUrl()` (`AUTH_URL` / `NEXT_PUBLIC_SITE_URL`). Default OG image (civic / light, §7). Complements TECH-11. Cookie-based i18n only — no URL locale / hreflang (NFR-04 unchanged). Static legal/FAQ pages may rely on root defaults (no separate per-page meta requirement) |
| TECH-16 | **`robots.txt`** via Next.js `app/robots.ts`: allow public routes; disallow auth-only paths; declare Sitemap URL | must | accepted | Disallow `/my-votes`, `/polls/new`, `/polls/*/edit`. Sitemap: `{site}/sitemap.xml`. Do **not** block common AI crawlers by default |
| TECH-17 | **Published poll** pages expose rich SEO metadata and a **canonical URL** preferring the alias | must | accepted | `generateMetadata`: title = question; description = truncated poll description (or short fallback); `alternates.canonical` = alias-preferred `pollHref` absolute URL. Open Graph / Twitter mirror title + description |
...
| Account created | `joined` | ISO datetime (or parseable date) of FSMeet account creation — used for roster transparency (FR-PO-035/036), not a gate |
...
| --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| **User**                                | FSMeet account + user type + demographics used for targeting (country, continent, age, gender, …) + profile trust/presence fields used for create/vote gates (`wffaId`, `verificationState`, social handles — §4.2) + account creation (`joined`) | FSMeet backend |
...
| **Creator ballot roster**               | Creator-only view of **who** voted and **for which** option(s) / abstention — transparency for the poll owner; may show voter **account age** vs poll creation (FR-PO-035/036)                                                                 | Voice          |
...
| FR-PO-035  | In the creator ballot roster UI (“Who voted”), show each voter’s **account age** derived from FSMeet `joined` (account creation). Display as **whole days** while under one year; otherwise as **whole years**. **Highlight** the voter when the account is **younger than the poll** (`joined` after poll `created_at`) | must     | accepted | Extends FR-PO-034; missing/unparseable `joined` → omit age, no highlight; comparison uses poll creation time, not `start_at` / `end_at` |
| FR-PO-036  | The ballot roster **CSV** includes the voter’s account creation datetime (`joined`) and a dedicated column flagging whether the account is **younger than the poll** (same rule as FR-PO-035)                                                                 | must     | accepted | Extends FR-PO-034; columns e.g. `account_joined` + `account_newer_than_poll` (`yes`/`no` or empty if `joined` unknown); highlight = explicit flag column for spreadsheet filtering |
